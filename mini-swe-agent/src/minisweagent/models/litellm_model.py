...
class LitellmModel:
    ...
    def _query(self, messages: list[dict[str, str]], **kwargs):
        try:
            ...
            if "openai" in self.config.model_name.lower() and "service_tier" not in merged_kwargs:
                merged_kwargs["service_tier"] = "flex"
            # return litellm.completion(
            #     model=self.config.model_name, messages=messages, **(self.config.model_kwargs | kwargs)
            # )
            return litellm.completion(
                model=self.config.model_name, messages=messages, **merged_kwargs
            )
